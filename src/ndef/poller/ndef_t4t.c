...
static ndefStatus ndefT4TTransceiveTxRx(ndefContext *ctx, rfalIsoDepApduTxRxParam *isoDepAPDU);
static ndefStatus ndefT4TReadAndParseCCFile(ndefContext *ctx);
...
/*******************************************************************************/
static ndefStatus ndefT4TReadNlen(ndefContext *ctx)
{
    ndefStatus           ret;
    uint8_t              nlenLen;
    uint8_t*             nLen;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerSelectNdefTagApplication(ndefContext *ctx)
{
    ndefStatus               ret;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerSelectFile(ndefContext *ctx, const uint8_t *fileId)
{
    ndefStatus               ret;
    rfalIsoDepApduTxRxParam  isoDepAPDU;
    ...
    {
        (void)rfalT4TPollerComposeSelectFileV1Mapping(isoDepAPDU.txBuf, fileId, NDEF_T4T_FID_SIZE, &isoDepAPDU.txBufLen);
    }
    else
    {
...
/*******************************************************************************/
ndefStatus ndefT4TPollerReadBinary(ndefContext *ctx, uint16_t offset, uint8_t len)
{
    ndefStatus               ret;
    rfalIsoDepApduTxRxParam  isoDepAPDU;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerReadBinaryODO(ndefContext *ctx, uint32_t offset, uint8_t len)
{
    ndefStatus               ret;
    rfalIsoDepApduTxRxParam  isoDepAPDU;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerReadBytes(ndefContext *ctx, uint32_t offset, uint32_t len, uint8_t *buf, uint32_t *rcvdLen)
{
    ndefStatus           ret;
    uint8_t              le;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerContextInitialization(ndefContext *ctx, const ndefDevice *dev)
{
...
/*******************************************************************************/
ndefStatus ndefT4TPollerNdefDetect(ndefContext *ctx, ndefInfo *info)
{
    ndefStatus           ret;
    uint8_t              nlenLen;
    ...
    /* Read NLEN/ENLEN TS T4T v1.0 7.2.1.11 */
    ret = ndefT4TReadNlen(ctx); /* Set ctx->state to NDEF_STATE_INITIALIZED, NDEF_STATE_READWRITE or NDEF_STATE_READONLY */
    if( ret != ERR_NONE )
    {
        /* Conclude procedure TS T4T v1.0 7.2.1.11 */
        return ret;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerReadRawMessage(ndefContext *ctx, uint8_t *buf, uint32_t bufLen, uint32_t *rcvdLen, bool single)
{
    ndefStatus           ret;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerWriteBinary(ndefContext *ctx, uint16_t offset, const uint8_t *data, uint8_t len)
{
    ndefStatus               ret;
    rfalIsoDepApduTxRxParam  isoDepAPDU;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerWriteBinaryODO(ndefContext *ctx, uint32_t offset, const uint8_t *data, uint8_t len)
{
    ndefStatus               ret;
    rfalIsoDepApduTxRxParam  isoDepAPDU;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerWriteBytes(ndefContext *ctx, uint32_t offset, const uint8_t *buf, uint32_t len, bool pad, bool writeTerminator)
{
    ndefStatus           ret;
    uint8_t              lc;
    uint32_t             lvOffset = offset;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerWriteRawMessageLen(ndefContext *ctx, uint32_t rawMessageLen, bool writeTerminator)
{
    ndefStatus           ret;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerWriteRawMessage(ndefContext *ctx, const uint8_t *buf, uint32_t bufLen)
{
    ndefStatus           ret;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerTagFormat(ndefContext *ctx, const ndefCapabilityContainer *cc, uint32_t options)
{
    NO_WARNING(ctx);
    NO_WARNING(cc);
    NO_WARNING(options);
...
/*******************************************************************************/
ndefStatus ndefT4TPollerCheckPresence(ndefContext *ctx)
{
    rfalIsoDepApduTxRxParam  isoDepAPDU;
    ReturnCode               ret;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerCheckAvailableSpace(const ndefContext *ctx, uint32_t messageLen)
{
    uint8_t              nlenLen;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerBeginWriteMessage(ndefContext *ctx, uint32_t messageLen)
{
    ndefStatus           ret;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerEndWriteMessage(ndefContext *ctx, uint32_t messageLen, bool writeTerminator)
{
    ndefStatus           ret;
...
/*******************************************************************************/
ndefStatus ndefT4TPollerSetReadOnly(ndefContext *ctx)
{
    NO_WARNING(ctx);
    return ERR_NOTSUPP;
}
